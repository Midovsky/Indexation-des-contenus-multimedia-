...
#define RACINE "training/"
#define RACINETEST "test/"
//Distance  entre deux vecteurs individu1 et individu2
...
int main(int argc, char** argv)
{
    ...
    if (argc==3)
    {
        ...
        vector<cv::String> fn;
        vector<cv::String> fntest;
        glob("test/*.png", fntest, false);
        glob("training/*.png", fn, false);
        vector<Mat> images;
        ...
        size_t counttest = fntest.size(); //number of png files in images folder
        ...
        for (size_t i=0; i<counttest; i++)
        {
            ...
            imagestest.push_back(imread(fntest[i]));
            //int* vecteurBase= histogrammeCouleurEchantillon(images[i],const_cast<char*>(fn[i].c_str()),nbreNiveaux) ;
            //distances.push_back(dist_histo_couleur(vecteur,vecteurBase,nbreNiveaux));

        }

        std::map<string,int> classesattribdoc,classescorrectattribdoc,classesappartdoc;

        for (size_t k=0; k<counttest; k++)
        {
            int* vecteur= histogrammeCouleurEchantillon(imagestest[k],const_cast<char*>(fntest[k].c_str()),nbreNiveaux) ;
            size_t count = fn.size(); //number of png files in images folder
            // int j=0;
            vector<float> distances;
            //calcul du nbre de document appartenant à chaque classe
            ...
            for (size_t i=0; i<count; i++)
            {
                ...
                //   cout  <<  " fn " << fn[i] << "\n";
                //calcul de distance dist_histo_couleur
                int* vecteurBase= histogrammeCouleurEchantillon(images[i],const_cast<char*>(fn[i].c_str()),nbreNiveaux) ;
                ...
                /* cv::String classeIndividu;
                 ...
                 if(classesappartdoc[classeIndividu]!=NULL)
                 {
                     classesappartdoc[classeIndividu]=classesappartdoc[classeIndividu]+1;

                 }
                 else
                 {
                     classesappartdoc[classeIndividu]=1;
                 }
                */
            }
...
//pos_min is short for position of min
            ...
            for (int i1=0; i1 < count1-1; i1++)
            {
                pos_min = i1;//set pos_min to the current index of array

                for (int j1=i1+1; j1 < count1; j1++)
                {

                    if (distances[j1] < distances[pos_min])
                        pos_min=j1;
                ...
                if (pos_min != i1)
                {
                    temp = distances[i1];
                    tempfn = fn[i1];
                    distances[i1] = distances[pos_min];
...
// cout <<  " nbre d'élément de la classe obj1 " << filename <<  " classe: " << classes["obj1"] << "\n";
            ...
            cv::String classeIndividuExperee;
            char* ss;
            string fichiertest=fntest[k];
            ss = strtok (const_cast<char*>(fichiertest.c_str()),"/");
            ss = strtok (NULL, "/");

            ss = strtok (ss,"__");
            classeIndividuExperee=ss;

            if(classesappartdoc[classeIndividuExperee]!=NULL)
            {
                classesappartdoc[classeIndividuExperee]=classesappartdoc[classeIndividuExperee]+1;

            }
            else
            {
                classesappartdoc[classeIndividuExperee]=1;
            }
            //  classesappartdoc
            if(classesattribdoc[classeIndividu]!=NULL)
            {
                classesattribdoc[classeIndividu]=classesattribdoc[classeIndividu]+1;

            }
            else
            {
                classesattribdoc[classeIndividu]=1;
            }
            if(strcmp(classeIndividu.c_str(),classeIndividuExperee.c_str())==0)
            {
                if(classescorrectattribdoc[classeIndividu]!=NULL)
                {
                    classescorrectattribdoc[classeIndividu]=classescorrectattribdoc[classeIndividu]+1;

                }
                else
                {
                    classescorrectattribdoc[classeIndividu]=1;
                }
            }

            //cout <<  " classification de l'image en entrée " << fntest[k] <<  " classe " << classeIndividu << "\n";
        }

//calcul de performance

        vector<string> keys;
        for (std::map<string,int>::iterator it=classesappartdoc.begin(); it!=classesappartdoc.end(); ++it)
        {
            //std::cout << it->first << " => " << it->second << '\n';
            /*if(it->second > max)
            {
        ...
        for(int t=0; t<keys.size(); t++)
        {
           ...
           // cout << "classescorrectattrib: " << classescorrectattribdoc[keys[t]] <<"\n";
            cout << "rappel: " << keys[t] <<" " <<(float) classescorrectattribdoc[keys[t]]*100/classesattribdoc[keys[t]] <<"%" <<"\n";
            cout << "precision: " << keys[t]<<" " << (float)classescorrectattribdoc[keys[t]]*100/classesappartdoc[keys[t]] <<"%" <<"\n";
